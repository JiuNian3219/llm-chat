...
COZE_TITLE_BOT_ID=你的标题生成 Bot ID（可选，不填则使用 COZE_BOT_ID）
```
...
COZE_BOT_ID=你的 Bot ID
COZE_TITLE_BOT_ID=你的标题生成 Bot ID（可选）
...
| `COZE_BOT_ID` | 主对话 Bot ID |
| `COZE_TITLE_BOT_ID` | 标题生成 Bot ID（可选，不填则使用 COZE_BOT_ID） |
| `CORS_ORIGIN` | 浏览器访问地址，如 `http://你的IP`（端口 80）或 `http://你的IP:端口`（可选，缺省 `http://localhost`） |
| `APP_PORT` | 对外暴露端口（可选，缺省 80） |

**首次部署前**：服务器需安装 Docker 与 Docker Compose。部署目录为 `~/llm-chat`。拉取 GHCR 镜像使用 workflow 自带的 `GITHUB_TOKEN`，无需额外配置。
